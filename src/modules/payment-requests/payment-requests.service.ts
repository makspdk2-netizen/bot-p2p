...
export class PaymentRequestsService implements OnModuleDestroy {
  ...
  async createRequest(
    ...
    if (!target) {
      throw new Error('Активная карта пользователя не найдена');
    }
    ...
    const request = await this.prisma.paymentRequest.create({
      data: {
        ...
        amount,
        deadline: new Date(
          Date.now() + PAYMENT_REQUEST_MINUTES * 60_000,
        ),
      },
      include: {
        user: true,
  ...
  async acceptFromUser(ctx: Context, requestId: bigint) {
    ...
    if (!request || request.status !== 'pending_user') {
      return;
    }

    if (ctx.from?.id !== Number(request.user.telegramId)) {
      return;
    }

    await this.completeAndDebit(
      request.id,
      'Пользователь подтвердил поступление',
    );
  ...
  async rejectFromUser(ctx: Context, requestId: bigint) {
    ...
    await ctx
      .editMessageText(
        this.proofRequestText(request.code),
      )
      .catch(() => undefined);
  }
  ...
  async receiveUserProof(ctx: Context) {
    ...
    if (!media) {
      await ctx.reply(
        '📎 Отправьте один файл: видео из приложения банка или PDF-выписку.',
      );
    ...
    await ctx.reply(
      '✅ Доказательство отправлено на проверку. Ожидайте решения.',
    );
  ...
  async startAdminMessage(
    ...
    if (
      !request ||
      ['completed', 'rejected', 'expired'].includes(
        request.status,
      )
    ) {
      return;
    ...
    await this.redis.setSession(ctx.from!.id, {
      step: 'admin_payment_request_message',
      data: {
    ...
    await ctx.reply(
      `💬 Введите сообщение пользователю по заявке ${request.code}:`,
    );
  }
  ...
  async receiveAdminMessage(
    ...
    if (!this.isAdmin(ctx) || !ctx.from) {
      return false;
    }

    const requestId = await this.readAdminRequestId(
      ctx.from.id,
    );
    ...
    await this.prisma.paymentRequestMessage.create({
      data: {
        requestId,
        senderType: 'admin',
        text: normalized,
    ...
    await this.bot?.api.sendMessage(
      Number(request.user.telegramId),
      `💬 Сообщение по заявке ${request.code}\n\n${normalized}`,
    );
  ...
  async rejectByAdmin(
    ...
    const request =
      await this.prisma.paymentRequest.updateMany({
        where: {
          id: requestId,
          status: {
            in: ['under_review', 'awaiting_proof'],
          },
        },
        data: {
          status: 'rejected',
    ...
    if (request.count === 1) {
      await ctx
        .editMessageText(
          '❌ Заявка отклонена. Списание не выполнено.',
        )
        .catch(() => undefined);
    }
  }
  ...
  private async expireRequests() {
    const expired =
      await this.prisma.paymentRequest.findMany({
        where: {
          status: {
            in: ['pending_user', 'awaiting_proof'],
          },
          deadline: {
            lte: new Date(),
          },
    ...
    for (const request of expired) {
      ...
      await this.debit(
        request.id,
        Number(request.amount),
        'Заявка завершена по истечении 15 минут',
      );
  ...
  private async completeAndDebit(
    requestId: bigint,
    description: string,
  ) {
    const request =
  ...
  private async debit(
    ...
    // Здесь нужен именно внутренний userId PostgreSQL.
  ...
  private async notifyAdmin(requestId: bigint) {
    ...
    await this.bot.api.sendMessage(
      adminId,
      `⚠️ Доказательство по заявке ${request.code}
...
Карта: ${request.requisite.bank || 'Банк'} ${
      ...
      }`,
      {
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: '✅ Подтвердить и списать',
                callback_data:
                  `payment_request_confirm:${request.id}`,
              },
              {
                text: '💬 Написать',
                callback_data:
                  `payment_request_message:${request.id}`,
              },
              {
                text: '❌ Отклонить',
                callback_data:
                  `payment_request_admin_reject:${request.id}`,
              },
            ],
          ],
        },
  ...
  private async sendRequestToUser(request: any) {
    if (!this.bot) {
      throw new Error('Бот ещё не подключён');
    }
            ...
            [
              {
                text: '✅ Да, поступили',
                callback_data:
                  `payment_request_accept:${request.id}`,
              },
              {
                text: '❌ Нет, не поступили',
                callback_data:
                  `payment_request_reject:${request.id}`,
              },
            ],
          ],
        },
...
Если за 15 минут ничего не выбрать, заявка завершится автоматически.`;
  }

  private proofRequestText(code: string) {
    return `📎 По заявке ${code} загрузите видео из приложения банка или PDF-выписку (один файл на выбор) в течение 15 минут.

Если за 15 минут ничего не загрузить, заявка завершится автоматически.`;
  }
...
Списано ${amount.toFixed(2)} RUB с баланса Epic P2P.`;
  ...
  private async readAdminRequestId(
    userId: number,
  ): Promise<bigint | null> {
    const session =
      await this.redis.getSession(userId);
